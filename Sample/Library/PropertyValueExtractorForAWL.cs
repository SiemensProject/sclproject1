...
namespace Library
{
    public class PropertyValueExtractorForAWL
    {/// <summary>
    ...
    /// <returns></returns>
            ...
            for(int i=0;i<Contents.Count();i++)
            {
                ...
                if (Contents[i].Contains(key + ":=" + "'" + value + "'") || Contents[i].Contains(key + " :=" + "'" + value + "'") || Contents[i].Contains(key + ":= " + "'" + value + "'") || Contents[i].Contains(key + " := " + "'" + value + "'"))
                {
                   ...
                   try {
                           ...
                           if (Contents[i].Contains("REAL"))
                            {
                                DefaultValueAssigner(Contents[i], props, dotNetDataTypes[0], edc);
                            }
                            else if (Contents[i].Contains("BOOL"))
                            {
                                DefaultValueAssigner(Contents[i], props, dotNetDataTypes[1], edc);
                            }
                            else if (Contents[i ].Contains("AnaValFF") || Contents[i].Contains("AnaVal"))
                            {
                                edc.dotNetDataType = dotNetDataTypes[0];
                                props.defaultValue = "0.0";
                            }
                            else if (Contents[i].Contains("DigValFF") || Contents[i].Contains("DigVal"))
                            {
                                edc.dotNetDataType = dotNetDataTypes[1];
                                props.defaultValue = "false";
                            }
                            else if (Contents[i].Contains("INT"))
                            {
                                DefaultValueAssigner(Contents[i], props, dotNetDataTypes[3], edc);
                            }
                            else if (Contents[i].Contains("STRING[32]"))
                            {
                                edc.dotNetDataType = dotNetDataTypes[2];
                            }
                            else if (Contents[i].Contains("BYTE"))
                            {
                                DefaultValueAssigner_hexatodecimal(Contents[i], props, dotNetDataTypes[5], edc);
                            }
                            else if (Contents[i].Contains("DWORD"))
                            {
                                DefaultValueAssigner_hexatodecimal(Contents[i], props, dotNetDataTypes[4], edc);
                            }
                            //else if (Contents[i].Contains("STRUCT"))
                            //{
        ...
        public void DefaultValueAssigner(string line, Property props, string dotNetdataType, EDC edc)
        {
            ...
            string[] dvalue = req[0].Split('=');
            string[] values = dvalue[dvalue.Length - 1].Split(';');
